...
%%
display('Saving post-processed image stack')
im_filenew=[viddir,dataset,'_IMAGEsmtopadjfiltmask.mat'];
if exist(im_filenew,'file')==2
    load(im_filenew);
else
...
outdata=[viddir,dataset,'_3DR_hiso_',num2str(val),'.mat'];
if exist(outdata,'file')==2
    load(outdata);
else
    [X,Y,Z]=meshgrid(1:size(IMAGEsm,2),1:size(IMAGEsm,1),1:size(IMAGEsm,3));
    xx=single(X);yy=single(Y);zz=single(Z);clear X Y Z
    fv=isosurface(xx,yy,zz,IMAGEsm,val,'verbose');
    outstl=[viddir,dataset,'_3DR_',num2str(val),'.stl'];
    stlwrite(outstl,fv);
...
outfile=[viddir,dataset,'_3DR_topadjfiltmask_',num2str(val),'.mp4'];
